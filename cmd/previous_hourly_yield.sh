...
echo "SELECT imported - exported
      FROM (
        SELECT (SELECT COALESCE(MAX(measurement)-MIN(measurement), 0)
                ...
                WHERE instant >= strftime('%s', strftime('%Y-%m-%dT%H:00:00Z', Datetime('now', '-'||(strftime('%M', 'now') + 1)||' minutes')))) exported,
               (SELECT COALESCE(MAX(measurement)-MIN(measurement), 0)
                FROM total_power_import_kwh
                WHERE instant >= strftime('%s', strftime('%Y-%m-%dT%H:00:00Z', Datetime('now', '-'||(strftime('%M', 'now') + 1)||' minutes')))) imported)"\
 | sqlite3 "$dbfile"\
